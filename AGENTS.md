...
- **Cloud sync** — Fully removed. No Supabase, no auth, no remote database.
- **Payments/Polar/Flouci** — Fully removed. Open source, free forever.
- **Google OAuth / Google Calendar / Google Tasks** — Fully removed.
- **Oracle branding** — Renamed to "Cyberia AI" everywhere.
- **`isOnboarding`** field — Removed from Space/Stack types.
- **`scatterThoughts()`** — Removed (was adding random jitter to positions).
- **`performanceMode`** — Removed (always full quality).
- **`published_spaces`** table — Removed.
- **`FilterPanel.tsx`**, `Contact.tsx`, `NodeMenu.tsx`, `SidebarLayout.tsx` — Deleted (orphan components).
- **`errorParser.ts`**, `i18n.ts`, `locales/` — Deleted.
- **`storageUrl`** — Deprecated field in Dexie schema. Use signed URLs on-demand or local blobs instead.
- **`clearWorkspaceData()`** — Anti-pattern that only clears Zustand memory (doesn't touch IndexedDB). Use `deleteData()` instead.
