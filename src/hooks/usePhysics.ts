...
export const usePhysics = (
  ...
  // Sync physics state with thoughts from store
  useEffect(() => {
    ...
    // Remove deleted thoughts from physics state
        ...
        if (moved) {
          ...
          if (mode === 'kanban') {
             ...
             sorted.forEach((t, index) => {
                 ...
                 if (mapEntry) {
                     if (t.id === id) mapEntry.status = status;
                     mapEntry.order = index;
                 }

                 if (t.id === id) {
                     updateThought(t.id, { status, order: index, x: p?.x, y: p?.y });
                 } else {
                     if (t.order !== index) {
                         updateThought(t.id, { order: index });
                     }
                 }
             });
             ...
             if (lastMouseX < mainLeft) {
                 ...
                 if (gridX >= 0 && gridY >= 0) {
                     ...
                     if (col >= 0 && col < 7 && row >= 0 && row < 6) {
                         ...
                         const dayIndex = row * 7 + col - startOffset;
  ...
  const loop = useCallback(() => {
    ...
    if (mode === 'kanban') {
       ...
       processColumn(todo, 'todo');
       processColumn(doing, 'doing');
       processColumn(done, 'done');
      ...
      scheduled.forEach((t) => {
        ...
        if (tDate.getFullYear() === year && tDate.getMonth() === month) {
            ...
            target.x = mainLeft + col * cellWidth + cellWidth / 2;
            target.y = topPadding + row * cellHeight + cellHeight / 2;
            ...
            target.y = window.innerHeight + 500;
            target.scale = 0;
        }
      ...
      ids.forEach((id) => {
        ...
        const targetScale = 1 + prioLevel * 0.05;

        p.vx += (window.innerWidth / 2 - p.x) * (GRAVITY * gravityMultiplier);
        p.vy += (window.innerHeight / 2 - p.y) * (GRAVITY * gravityMultiplier);

        const el = elements.current.get(id);
        const nHeight = el?.offsetHeight || 120;
        const nRadius = Math.max(100, nHeight / 2);

        ids.forEach((otherId) => {
          if (id === otherId) return;
          const otherP = state.get(otherId)!;
          const otherT = thoughtMap.current.get(otherId);
          if (!otherT) return;
          ...
          const otherRadius = Math.max(100, otherHeight / 2);
          const minDistance = (nRadius + otherRadius) * 1.1;
          ...
          let repulsionPower = REPULSION;
          if (d < minDistance) {
            repulsionPower *= minDistance / d;
          }

          const force = Math.min((repulsionPower * repulsionMultiplier) / distSq, 60);
          p.vx += (dx / d) * force;
          p.vy += (dy / d) * force;
    ...
    if (ctx && canvasRef.current) {
      ...
      for (let i = 0; i < ids.length; i++) {
        const idA = ids[i];
        const tA = thoughtMap.current.get(idA);
        const pA = state.get(idA);
        if (!tA || !pA) continue;

        for (let j = i + 1; j < ids.length; j++) {
            const idB = ids[j];
            const tB = thoughtMap.current.get(idB);
            const pB = state.get(idB);
            if (!tB || !pB) continue;
