  ...
  if (status !== 'authenticated' || !user) {
    return (
      <button
        ...
        className="h-[48px] px-5 glass rounded-2xl border border-white/5 text-[var(--text-dimmed)] hover:text-white hover:bg-white/5 transition-all flex items-center gap-3 group pointer-events-auto shadow-sm"
      >
      ...
      >
        ...
        <div className="hidden sm:flex items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            {user.plan === 'pro' && (
              <span className="px-1 py-0.5 rounded-[4px] bg-blue-500/20 border border-blue-500/30 text-blue-400 text-[7px] font-black uppercase tracking-wide">
                PRO
              </span>
            )}
          </div>
            ...
            <div className="p-5">
              ...
              <div className="flex items-center gap-4 mb-6">
                ...
                <div className="flex-1 overflow-hidden">
                  <h4 className="text-[11px] font-black uppercase tracking-wide text-white truncate">{user.name}</h4>
                  <p className="text-[9px] font-medium text-[var(--text-muted)] truncate">{user.email}</p>
                  
                  {user.plan === 'pro' && user.expiryDate && (
                    <p className="text-[8px] font-bold text-blue-400/80 uppercase tracking-widest mt-1">
                      Pro • Ends {new Date(user.expiryDate).toLocaleDateString(undefined, { 
                ...
                {user.plan === 'pro' ? (
                  user.paymentProvider === 'polar' && (
                    <button 
                      ...
                      className="w-full flex items-center justify-center gap-2 px-4 h-9 rounded-xl bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 transition-all border border-blue-500/20 shadow-inner group"
                    >
                      <CreditCard className="w-3.5 h-3.5 group-hover:scale-110 transition-transform" />
                      <span className="text-[9px] font-black uppercase tracking-[0.2em]">Manage Plan</span>
                  ...
                  <button 
                    ...
                    className="w-full flex items-center justify-center gap-2.5 px-4 h-9 rounded-xl bg-amber-600/10 hover:bg-amber-600/20 text-amber-400 transition-all border border-amber-500/20 shadow-inner group"
                  >
              ...
              <div className="space-y-4 mb-6">
                <p className="text-[9px] font-black uppercase tracking-wide text-[var(--text-muted)]">Sync & Storage</p>
                ...
                <div className="px-1">
                  <div className="flex justify-between items-center mb-1.5">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[8px] font-black uppercase tracking-wide text-[var(--text-dimmed)]">Cloud Storage</span>
                    </div>
                    <span className={cn(
              ...
              <div className="pt-4 border-t border-white/5 space-y-2">
                <button 
                  ...
                  className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-red-500/10 text-[9px] font-black uppercase tracking-wide text-red-400 transition-colors group"
                >
                ...
                <button 
                  ...
                  className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-white/5 text-[9px] font-black uppercase tracking-wide text-[var(--text-primary)] transition-colors group"
                >
