...
import { Plus, ArrowLeft, File as FileIcon } from 'lucide-react';
...
export const ActionFAB: React.FC<ActionFABProps> = ({ isReadOnly, handleAddThought }) => {
  ...
  return (
    <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[10000] pointer-events-none flex items-center gap-4 transition-all duration-300">
      <input 
        type="file" 
        className="hidden" 
        ref={fileInputRef} 
        onChange={(e) => {
          ...
          if (files && files.length > 0) {
            // Viewport handleDrop handles the drop event, but we can reuse the logic
            // or trigger a custom event. For now, let's keep it simple.
