    ...
    <div className="max-w-full flex items-center h-[52px] glass rounded-full shadow-2xl transition-all pointer-events-auto border border-white/5 px-2">
      {isReadOnly ? (
        ...
        <div className="flex items-center gap-1 h-full min-w-max">
          ...
          <div id="space-switcher-list" className="flex items-center gap-1 overflow-x-auto custom-scroll pb-1 max-w-[450px] 2xl:max-w-[750px] pointer-events-auto" onWheel={(e) => { e.stopPropagation(); e.currentTarget.scrollLeft += e.deltaY; }}>
            {spaces.map((space) => {
              const isActive = space.id === activeSpaceId;
              return (
                <button key={space.id} onClick={() => setActiveSpace(space.id)} className={cn("px-3 h-9 min-w-[110px] md:min-w-[120px] rounded-full text-[10px] uppercase font-black tracking-widest flex-shrink-0 transition-all duration-500 flex items-center justify-center gap-2", isActive ? "bg-white/10 text-white border border-white/10 shadow-[0_0_20px_rgba(255,255,255,0.05)]" : "text-slate-500 hover:text-slate-300 hover:bg-white/[0.03] border border-transparent")}>
                  <div className={cn("w-1 h-1 rounded-full bg-[var(--accent-secondary)] shadow-[0_0_8px_var(--accent)]", getStatusColor(space, isReadOnly))} />
          ...
          {spaces.length < limits.MAX_SPACES && (
            <><div className="w-[1px] h-3 bg-white/10 mx-1"></div><button onClick={handleCreateSpace} className="w-9 h-9 rounded-full text-slate-600 hover:text-white hover:bg-white/10 transition-all flex-shrink-0 flex items-center justify-center border border-white/10 border-dashed"><Plus className="w-3.5 h-3.5" /></button></>
          )}
        </div>
      )}
    </div>
