  ...
  <div className="flex items-center gap-2 flex-shrink-0 pointer-events-none z-20">
    <div className="flex items-center h-[48px] p-1.5 glass rounded-2xl shadow-2xl transition-all pointer-events-auto border border-white/5">
      {[
        ...
        return (
          <button key={mode.id} onClick={() => setViewMode(mode.id as 'spatial' | 'kanban' | 'calendar')} className={cn("px-2.5 md:px-4 h-full rounded-xl transition-all duration-300 flex items-center gap-2 group/mode", isActive ? "bg-white/10 text-white shadow-xl" : "text-slate-500 hover:text-slate-300 hover:bg-white/[0.03]")}>
            ...
            <span className={cn("text-[8px] md:text-[9px] font-black uppercase tracking-widest transition-all overflow-hidden whitespace-nowrap hidden lg:inline", isActive ? "max-w-[60px] opacity-100" : "max-w-0 opacity-0")}>{mode.id}</span>
          </button>
        );
      })}
    </div>
