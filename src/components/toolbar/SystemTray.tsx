...
interface SystemTrayProps {
  ...
  theme: 'cyberia' | 'sakura' | 'neon' | null;
  setTheme: (val: 'cyberia' | 'sakura' | 'neon') => void;
  deferredPrompt: any;
  handleInstall: () => void;
  handleExport: () => void;
...
export const SystemTray: React.FC<SystemTrayProps> = ({ 
  ...
  <div className="fixed bottom-4 md:bottom-8 right-4 md:right-8 z-[9999] flex flex-col items-end gap-3 pointer-events-none system-tray-container">
    <div className={cn("glass p-4 md:p-5 rounded-[1.5rem] md:rounded-[2rem] border border-white/10 shadow-2xl flex flex-col gap-1 transition-all pointer-events-auto w-64 md:w-72 animate-in fade-in slide-in-from-bottom-2 duration-300", isSystemMenuOpen ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4 pointer-events-none")}>
      <div className="px-1 md:px-2 py-3 border-b border-white/5 mb-3"><p className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.2em] text-slate-500 mb-4">Workspace Theme</p><div className="grid grid-cols-3 gap-2">{(['cyberia', 'sakura', 'neon'] as const).map((id) => { const labels = { cyberia: 'Cyber', sakura: 'Sakura', neon: 'Neon' }; const colors = { cyberia: '#6366f1', sakura: '#fdb9c8', neon: '#10b981' }; return (<button key={id} onClick={() => setTheme(id)} className={cn("flex flex-col items-center gap-2.5 p-2.5 rounded-xl border transition-all", theme === id ? "bg-white/10 border-white/20 shadow-lg" : "border-transparent hover:bg-white/5")}><div className="w-4 h-4 rounded-full shadow-lg" style={{ backgroundColor: colors[id] }} /><span className={cn("text-[8px] md:text-[9px] font-bold uppercase tracking-widest", theme === id ? "text-white" : "text-slate-500")}>{labels[id]}</span></button>); })}</div></div>
