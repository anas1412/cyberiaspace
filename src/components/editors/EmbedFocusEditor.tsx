  ...
  const renderPlayer = () => {
    ...
    return (
      <div className="absolute inset-0 flex flex-col items-center justify-center bg-black overflow-hidden">
        {image ? (
          <div className="w-full h-full flex items-center justify-center p-4"><img src={image} alt="Content" className="max-w-full max-h-full object-contain rounded-xl shadow-2xl border border-white/5" /></div>
        ) : (
    ...
    <FocusEditorShell
      ...
      headerSubContent={
        <div className="flex items-center gap-2 mt-1 overflow-hidden">
          ...
          <p className="text-[9px] text-slate-500 uppercase font-black tracking-widest truncate opacity-60">{content}</p>
        </div>
      }
      headerActions={
        <a href={content} target="_blank" rel="noopener noreferrer" className="p-3 md:p-4 hover:bg-white/5 rounded-xl md:rounded-2xl text-slate-400 hover:text-white transition-all">
